...
void procesar_texto(Terminal* term)
{
    ...
    */
    ...
    // set notepad ++ x86 y x64
    strcpy(programfilesX64, getenv("PROGRAMFILES"));
    strcpy(npplusX64, programfilesX64);
    strcat(npplusX64, "\\notepad++\\notepad++.exe");

    strcpy(programfilesX86, getenv("ProgramFiles(x86)"));
    strcpy(npplusX86, programfilesX86);
    strcat(npplusX86, "\\notepad++\\notepad++.exe");

    // setear cual notepad++ abrir 
    // en primera instancia el de 64 
    // si no existe el de x86 
    if (npplusX64) {
        strcpy(npplus, npplusX64);
    }
    if (npplusX86) {
        strcpy(npplus, npplusX86);
    }
