...
// printclip visor
static bool reconfigPrinter = false;
    ...
    if (key_down && (keystate[VK_RMENU] & 0x80) == 0 && !wgs->compose_state) {
		...
		*/
		...
		if (left_alt && wParam == VK_F6) {
            reconfigPrinter = true;
			conf_cache_data(wgs);
			conf_set_str(wgs->conf, CONF_printer, PRINTCLIP_STRING);
		...
		if (left_alt && wParam == VK_F7) {
            reconfigPrinter = true;
			conf_cache_data(wgs);
			conf_set_str(wgs->conf, CONF_printer, VISOR_STRING);
		...
		/*Fin adicion de hotkeys printclip*/
